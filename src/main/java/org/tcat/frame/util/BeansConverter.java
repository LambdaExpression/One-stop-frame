...
import org.tcat.frame.annotation.Json;
...
public class BeansConverter {
    ...
    private static synchronized Map<String, PropertyDescriptor> getCachePropertyDescriptors(Class<?> clazz)
        ...
        if (map == null) {
            ...
            for (PropertyDescriptor descriptor : descriptors) {
                ...
                if (readMethod == null) {
                    try {
                        readMethod = clazz.getMethod("get" + name);
                        descriptor.setReadMethod(readMethod);
                    } catch (NoSuchMethodException | SecurityException e) {
                    }
                }
                ...
                if (writeMethod == null) {
                    try {
                        writeMethod = clazz.getMethod("set" + name,
                                descriptor.getPropertyType());
                        descriptor.setWriteMethod(writeMethod);
                    } catch (NoSuchMethodException | SecurityException e) {
                    }
    ...
    protected static <T> T convert(Object src, T dest, boolean setDefaultValForNull) throws BeanConverterException {
        ...
        try {
            ...
            Map<String, PropertyDescriptor> srcDescriptors = getCachePropertyDescriptors(srcClass);
            ...
            for (String key : keys) {
                ...
                if (writeMethod != null) {
                    ...
                    if (!matched) {
                        Field destField = getDeclaredField(destClass, key);
                        Field srcField = getDeclaredField(srcClass, key);
                        Json destJson = destField == null ? null : destField.getAnnotation(Json.class);
                        Json srcJson = srcField == null ? null : srcField.getAnnotation(Json.class);
                        if (value instanceof String && destJson != null) {
                            value = gson.fromJson((String) value, propertyType);
                        }
                        if (value != null && srcJson != null && propertyType == String.class) {
            ...
            for (Object constant : constants) {
                if (value instanceof String) {
                    ...
                    if (constant instanceof Enum) {
                        if ((Integer.valueOf(constant.toString())) == NumberUtils.toInt(value.toString())) {
                            value = constant;
                            break;
                        }
                    } else {
                    ...
                    if ((Boolean) value && ((Enum<?>) constant).ordinal() == 1) {
                        value = constant;
                        break;
                    } else if (!(Boolean) value && ((Enum<?>) constant).ordinal() == 0) {
                        value = constant;
                        break;
                    }
                }
